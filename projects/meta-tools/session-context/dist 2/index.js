...
function getLastSessionFromNotes() {
    try {
        ...
        for (let i = lastSessionStart; i < Math.min(lastSessionStart + 30, lines.length); i++) {
            if (i > lastSessionStart && lines[i].match(/^## Session \d+/))
                break;
...
// Tools
server.tool("session_start", "Initialize session context - call this at the start of each Claude Code session to load previous context", {}, async () => {
    try {
...
server.tool("session_get_last_summary", "Get the full summary from the last session for context continuity", {}, async () => {
    try {
        const lastSession = getLastSessionFromNotes();
        if (!lastSession) {
            return formatResponse({
...
server.tool("session_set_project", "Set the active project for this session (affects context loading and coaching)", {
...
server.tool("session_add_file", "Track a file being actively worked on (for context preservation)", {
...
server.tool("session_add_task", "Log a task being worked on or completed", {
...
server.tool("session_set_context", "Store arbitrary context data for cross-session persistence", {
...
server.tool("session_get_context", "Retrieve stored context data", {
...
server.tool("session_status", "Get current session status including active project, files, and tasks", {}, async () => {
    try {
...
server.tool("session_end", "Finalize session context before ending (call before /done or exit)", {
...
server.tool("session_info", "Get information about the session-context MCP server", {}, async () => {
    return formatResponse({
        name: 'session-context',
        version: '1.1.0',
...
server.tool("session_export_to_memory", "Export current session context to the memory graph in Supabase for long-term persistence", {
...
server.tool("session_import_from_memory", "Import context from memory graph - useful for resuming work on a project", {
    ...
    try {
        ...
        let sessions = data || [];
        if (projectName) {
            sessions = sessions.filter(s => s.metadata?.project === projectName ||
                s.observations?.some((o) => o.includes(projectName)));
        }
