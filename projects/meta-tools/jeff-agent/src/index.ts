...
const FAMILY_CALENDARS: Record<string, { id: string; owner: string; type: string }> = {
...
server.tool(
  "jeff_add_family_member",
  "Add or update a family member for calendar tracking",
  {
...
server.tool(
  "jeff_today",
  "Get today's overview: calendar events, tasks, and priorities for the family",
  {
...
server.tool(
  "jeff_week",
  "Get week-ahead overview with conflict detection",
  {
...
server.tool(
  "jeff_add_recurring",
  "Add a recurring item (birthday, anniversary, renewal, seasonal task)",
  {
...
server.tool(
  "jeff_list_upcoming",
  "List upcoming recurring items (birthdays, renewals, etc.)",
  {
...
server.tool(
  "jeff_checkin",
  "Log a PERMA wellbeing check-in",
  {
  ...
  async (args) => {
    ...
    try {
      ...
      return formatResponse({
        ...
        log: data,
        permaAverage,
        insight: permaAverage ? (
          permaAverage >= 8 ? 'Excellent! You\'re thriving.' :
          permaAverage >= 6 ? 'Good day. Keep nurturing what\'s working.' :
          permaAverage >= 4 ? 'Moderate. Consider what might boost your wellbeing.' :
          'Challenging day. Be kind to yourself and reach out if needed.'
        ) : 'Complete more PERMA dimensions for insights.'
      });
    } catch (error) {
      return formatResponse(`Error: ${error instanceof Error ? error.message : 'Unknown error'}`, true);
    }
...
server.tool(
  ...
  async (args) => {
    ...
    try {
      ...
      // Identify trends
      const trends: string[] = [];
      if (averages.positive_emotion >= 7) trends.push('Strong positive emotions');
      if (averages.stress_level >= 7) trends.push('Consider stress management techniques');
      if (averages.sleep_quality <= 5) trends.push('Sleep quality could use attention');
      if (averages.relationships >= 7) trends.push('Good social connections');
...
server.tool(
  "jeff_habit_status",
  "View all habits with streaks and today's status",
  {},
  async () => {
    if (!supabase) {
      return formatResponse('Supabase not configured', true);
...
server.tool(
  "jeff_personal_digest",
  "Generate a comprehensive personal/family daily digest with calendar, tasks, wellbeing, and habits",
  {},
  async () => {
    if (!supabase) {
      return formatResponse('Supabase not configured', true);
    ...
    try {
      ...
      const habitsCompleted = habits.filter(h =>
        habitLogs.find(l => l.habit_id === h.id && l.completed)
      ).length;
...
server.tool(
  "jeff_add_email_rule",
  "Add a new email triage rule for auto-categorization",
  {
...
server.tool(
  "jeff_list_email_rules",
  "List all email triage rules",
  {
...
server.tool(
  "jeff_apply_email_rules",
  "Apply email rules to classify an email and return matching rules",
  {
...
server.tool(
  "jeff_delete_email_rule",
  "Delete or deactivate an email rule",
  {
...
server.tool(
  "jeff_detect_conflicts",
  "Detect calendar conflicts across family calendars for a given time range",
  {
...
server.tool(
  "jeff_analyze_conflicts",
  "Analyze a list of calendar events and identify conflicts",
  {
          ...
          if (startA < endB && startB < endA) {
            ...
            conflicts.push({
              ...
              overlapMinutes: Math.round(
                (Math.min(endA, endB) - Math.max(startA, startB)) / 60000
              ),
...
server.tool(
  "jeff_habits_at_risk",
  "Get habits with streaks that are at risk of breaking (not completed today)",
  {
  ...
  async (args) => {
    ...
    try {
      ...
      const milestones = habits?.filter(h =>
        [7, 14, 21, 30, 60, 90, 100, 365].includes(h.current_streak)
      ).map(h => ({
        name: h.name,
        streak: h.current_streak,
...
server.tool(
  "jeff_generate_digest_payload",
  "Generate a structured digest payload for external delivery (n8n webhook, Telegram, email)",
  {
...
function formatDigestAsMarkdown(payload: any, isTelegram: boolean): string {
  ...
  if (summary.overdueTasks > 0 || summary.urgentTasks > 0 || summary.emailsNeedingResponse > 0) {
    ...
    if (summary.overdueTasks > 0) lines.push(`• ${summary.overdueTasks} overdue task(s)`);
    if (summary.urgentTasks > 0) lines.push(`• ${summary.urgentTasks} urgent task(s)`);
    if (summary.emailsNeedingResponse > 0) lines.push(`• ${summary.emailsNeedingResponse} email(s) need response`);
    if (summary.habitsAtRisk > 0) lines.push(`• ${summary.habitsAtRisk} habit streak(s) at risk`);
    lines.push('');
  }
  ...
  if (payload.priorities.dueToday.length > 0) {
    ...
    payload.priorities.dueToday.forEach((t: any) => {
  ...
  if (payload.priorities.habitsAtRisk.length > 0) {
    ...
    payload.priorities.habitsAtRisk.forEach((h: any) => {
  ...
  if (payload.upcoming.length > 0) {
    ...
    payload.upcoming.forEach((r: any) => {
...
server.tool(
  "jeff_perma_trends",
  "Analyze PERMA wellbeing trends over time with dimension-specific insights",
  {
      ...
      dimensions.forEach(dim => {
        ...
        if (values.length > 0) {
          ...
          if (values.length >= 4) {
            ...
            if (diff > 0.5) trends[dim] = 'improving';
            else if (diff < -0.5) trends[dim] = 'declining';
...
function getDimensionRecommendations(dimension: string, score: number): string[] {
...
function getOverallInsight(score: number | null, lowestDimension: string | undefined, trends: Record<string, string>): string {
  if (!score) return 'Complete more check-ins to get personalized insights.';
...
server.tool(
  "jeff_upcoming_with_actions",
  "Get upcoming recurring items with suggested actions and context",
  {
    ...
    try {
      ...
      const enrichedItems = data?.map(item => {
        const daysUntil = Math.ceil(
          (new Date(item.next_occurrence).getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
        );
...
server.tool(
  "jeff_project_context",
  "Get recent project activity and context for intelligent follow-ups",
  {
  ...
  async (args) => {
    ...
    try {
      ...
      const momentum: 'high' | 'medium' | 'low' | 'stale' =
        activityCount >= 10 ? 'high' :
...
server.tool(
  "jeff_log_project_activity",
  "Log an activity for a project (for tracking momentum)",
  {
    project_id: z.string(),
...
server.tool(
  ...
  "Store an AI-generated summary for an email thread",
  {
    thread_id: z.string().describe('Jeff thread UUID'),
...
server.tool(
  "jeff_quick",
  "Get only urgent/critical items for quick review",
  {},
  async () => {
    if (!supabase) {
      return formatResponse('Supabase not configured', true);
...
server.tool(
  "jeff_classify_email",
  "Classify an email using local LLM (DeepSeek R1) or rule-based matching. Returns classification, confidence, and suggested action.",
  {
  ...
  async (args) => {
    ...
    try {
      ...
      if (ruleMatches && ruleMatches.length > 0) {
        ...
        const ruleToClassification: Record<string, string> = {
        ...
        const ruleToAction: Record<string, string> = {
...
{"classification":"spam|marketing|fyi|needs_response|urgent","confidence":0.0,"suggested_action":"archive|read|reply|create_task|escalate","reasoning":"brief explanation"}`;
          ...
          const ollamaResult = await response.json() as any;
...
server.tool(
  "jeff_propose_action",
  "Generate an AI action proposal for a task or email thread. Creates a proposal with optional draft content.",
  {
  ...
  async (args) => {
    ...
    try {
      ...
      if (source_type === 'email_thread') {
        ...
        if (!thread) return formatResponse('Email thread not found', true);
        ...
        const { data: task } = await supabase.from('jeff_tasks')
          .select('*').eq('id', source_id).single();
        if (!task) return formatResponse('Task not found', true);
        sourceContext = task;
        if (!inferredAction) {
...
server.tool(
  "jeff_execute_action",
  "Execute an action proposal: accept, dismiss, snooze, or modify. For email replies, returns send instructions for unified-comms.",
  {
...
server.tool(
  "jeff_get_email_context",
  "Get full context for an email thread: thread data, sender history, related tasks, associations, and classification analytics.",
  {
