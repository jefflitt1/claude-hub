...
// Initialize caches
const supabaseCache = new SimpleCache(100, SUPABASE_CACHE_TTL);
const gdriveCache = new SimpleCache(50, GDRIVE_CACHE_TTL);
const n8nCache = new SimpleCache(30, N8N_CACHE_TTL);
...
// Supabase Tools
server.tool("l7_query", "Query data from L7 Partners Supabase database with caching", querySchema, async (args) => {
    if (!supabase) {
...
server.tool("l7_insert", "Insert data into L7 Partners Supabase database", insertSchema, async (args) => {
    if (!supabase) {
...
server.tool("l7_update", "Update data in L7 Partners Supabase database", updateSchema, async (args) => {
    if (!supabase) {
        return formatResponse('Supabase client not configured.', true);
    }
    try {
...
server.tool("l7_delete", "Delete data from L7 Partners Supabase database", deleteSchema, async (args) => {
    if (!supabase) {
        return formatResponse('Supabase client not configured.', true);
    }
    try {
...
server.tool("l7_sql", "Execute raw SQL query on L7 Partners database (use with caution)", sqlSchema, async (args) => {
    if (!supabase) {
        return formatResponse('Supabase client not configured.', true);
    }
    try {
...
server.tool("l7_list_tables", "List tables in L7 Partners Supabase database", listTablesSchema, async (args) => {
    if (!supabase) {
        return formatResponse('Supabase client not configured.', true);
    }
    try {
...
server.tool("l7_describe_table", "Get schema information for a table (columns, types, constraints)", {
    ...
    try {
      ...
      `;
        ...
        const primaryKeys = pkData?.map((r) => r.column_name) || [];
...
server.tool("l7_get_execution", "Get details of a specific n8n execution", {
...
// n8n Tools
server.tool("l7_workflow_trigger", "Trigger an n8n workflow via webhook", workflowTriggerSchema, async (args) => {
    try {
...
server.tool("l7_list_workflows", "List available n8n workflows", {
...
server.tool("l7_get_workflow", "Get details of a specific n8n workflow", {
    ...
    try {
        ...
        const result = {
            ...
            workflow: {
                ...
                nodeTypes: workflow.nodes?.map((n) => n.type).filter((v, i, a) => a.indexOf(v) === i) ?? []
            }
        };
        n8nCache.set(key, result);
        return formatResponse(result);
...
server.tool("l7_activate_workflow", "Activate an n8n workflow", {
...
server.tool("l7_deactivate_workflow", "Deactivate an n8n workflow", {
...
server.tool("l7_list_executions", "List recent n8n workflow executions", {
...
server.tool("l7_vector_search", "Search documents using semantic similarity (requires embedding from OpenAI/other provider)", vectorSearchSchema, async (args) => {
    if (!supabase) {
        return formatResponse('Supabase client not configured.', true);
    }
    try {
...
server.tool("l7_vector_search_with_embedding", "Search documents using a pre-computed embedding vector", {
    ...
    try {
        ...
        // Filter by threshold
        const results = (data || []).filter((r) => r.similarity >= threshold);
        return formatResponse({
...
server.tool("l7_vector_insert", "Insert a document chunk with its embedding into the vector store", vectorInsertSchema, async (args) => {
    if (!supabase) {
        return formatResponse('Supabase client not configured.', true);
    }
    try {
...
server.tool("l7_vector_stats", "Get statistics about the document embeddings store", {
...
server.tool("l7_doc_search", "Search L7 Partners Google Drive", docSearchSchema, async (args) => {
    try {
...
// Meta/Info Tools
server.tool("l7_info", "Get information about L7 Business meta-tool status and caches", {}, async () => {
    return formatResponse({
        name: 'l7-business',
        version: '2.0.0',
...
server.tool("l7_clear_cache", "Clear L7 Business caches", {
    target: z.enum(['all', 'supabase', 'gdrive', 'n8n']).optional().default('all')
}, async (args) => {
    const { target } = args;
    if (target === 'all' || target === 'supabase')
        supabaseCache.clear();
    if (target === 'all' || target === 'gdrive')
        gdriveCache.clear();
    if (target === 'all' || target === 'n8n')
        n8nCache.clear();
    return formatResponse({
