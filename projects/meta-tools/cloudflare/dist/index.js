...
// DNS RECORD TOOLS
// =============================================================================
const dnsRecordTypes = ['A', 'AAAA', 'CNAME', 'MX', 'TXT', 'NS', 'SRV', 'CAA', 'PTR'];
// Tool: List DNS records
server.tool("list_dns_records", "List all DNS records for the configured zone. Optionally filter by type or name.", {
...
// Tool: Get DNS record
server.tool("get_dns_record", "Get a specific DNS record by ID", {
...
server.tool("update_dns_record", "Update an existing DNS record", {
...
server.tool("delete_dns_record", "Delete a DNS record by ID", {
...
// =============================================================================
// Tool: List tunnels
server.tool("list_tunnels", "List all Cloudflare tunnels for the account", {
    ...
    try {
        ...
        let tunnels = data.result.map((t) => ({
            ...
            activeConnections: t.connections?.filter((c) => !c.is_pending_reconnect).length || 0
        }));
        // Filter by status if requested
        if (args.status) {
            tunnels = tunnels.filter((t) => t.status === args.status);
        }
        return formatResponse({
...
server.tool("get_tunnel", "Get detailed information about a specific tunnel", {
...
server.tool("get_tunnel_config", "Get the configuration for a remotely-managed tunnel", {
...
// Tool: Create tunnel
server.tool("create_tunnel", "Create a new Cloudflare tunnel", {
...
// Tool: Delete tunnel
server.tool("delete_tunnel", "Delete a Cloudflare tunnel (must have no active connections)", {
...
server.tool("update_tunnel_config", "Update the configuration for a remotely-managed tunnel (add/remove ingress rules)", {
    tunnelId: z.string().describe('The tunnel ID'),
...
server.tool("cleanup_tunnel_connections", "Clean up stale connections for a tunnel", {
    tunnelId: z.string().describe('The tunnel ID')
}, async (args) => {
    try {
        if (!CLOUDFLARE_ACCOUNT_ID) {
...
server.tool("list_access_apps", "List all Cloudflare Access applications (Zero Trust)", {}, async () => {
    try {
        if (!CLOUDFLARE_ACCOUNT_ID) {
            return formatResponse('CLOUDFLARE_ACCOUNT_ID not configured', true);
        }
        const data = await cloudflareApi(`/accounts/${CLOUDFLARE_ACCOUNT_ID}/access/apps`);
        const apps = data.result.map((app) => ({
            id: app.id,
            name: app.name,
            domain: app.domain,
            type: app.type,
...
server.tool("get_access_app", "Get details of a specific Cloudflare Access application", {
...
server.tool("create_access_app", "Create a new Cloudflare Access application (self-hosted)", {
...
server.tool("update_access_app", "Update a Cloudflare Access application", {
    appId: z.string().describe('The Access application ID'),
    ...
    try {
        ...
        if (updates.domain !== undefined)
            body.domain = updates.domain;
        if (updates.sessionDuration !== undefined)
            body.session_duration = updates.sessionDuration;
        if (updates.autoRedirectToIdentity !== undefined)
            body.auto_redirect_to_identity = updates.autoRedirectToIdentity;
        if (updates.appLauncherVisible !== undefined)
            body.app_launcher_visible = updates.appLauncherVisible;
        if (Object.keys(body).length === 0) {
            return formatResponse('No updates provided', true);
        }
...
server.tool("delete_access_app", "Delete a Cloudflare Access application", {
...
server.tool("list_access_policies", "List all policies for a Cloudflare Access application", {
    appId: z.string().describe('The Access application ID')
}, async (args) => {
    try {
        if (!CLOUDFLARE_ACCOUNT_ID) {
        ...
        const policies = data.result.map((p) => ({
            id: p.id,
            name: p.name,
            decision: p.decision,
...
server.tool("create_access_policy", "Create a policy for a Cloudflare Access application", {
    appId: z.string().describe('The Access application ID'),
    ...
    try {
        ...
        if (args.precedence !== undefined)
            body.precedence = args.precedence;
        if (exclude.length > 0)
...
server.tool("create_ip_bypass_policy", "Create a bypass policy that allows specific IP addresses without authentication (ideal for Tailscale IPs)", {
    appId: z.string().describe('The Access application ID'),
...
server.tool("update_access_policy", "Update a policy for a Cloudflare Access application", {
    appId: z.string().describe('The Access application ID'),
    ...
    try {
        ...
        if (updates.decision !== undefined)
            body.decision = updates.decision;
        if (updates.precedence !== undefined)
            body.precedence = updates.precedence;
        if (Object.keys(body).length === 0) {
            return formatResponse('No updates provided', true);
        }
...
server.tool("delete_access_policy", "Delete a policy from a Cloudflare Access application", {
    appId: z.string().describe('The Access application ID'),
...
// INFO/STATUS TOOL
// =============================================================================
server.tool("cloudflare_info", "Get Cloudflare MCP server status and configuration info", {}, async () => {
