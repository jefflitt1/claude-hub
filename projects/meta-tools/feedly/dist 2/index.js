...
const cache = new SimpleCache(100, CACHE_TTL);
...
// Feedly API helper
async function feedlyApi(endpoint, options = {}) {
...
server.tool("feedly_profile", "Get your Feedly user profile information", {}, async () => {
    try {
...
server.tool("feedly_collections", "Get all your Feedly collections (categories) with their subscribed feeds", {}, async () => {
    try {
        ...
        const summary = collections.map((col) => ({
            ...
            feeds: col.feeds?.map((f) => ({ title: f.title, website: f.website })) || []
        }));
...
server.tool("feedly_subscriptions", "Get all your Feedly feed subscriptions", {}, async () => {
    try {
...
server.tool("feedly_unread_counts", "Get unread article counts for all your feeds and categories", {}, async () => {
    try {
        const counts = await feedlyApi('/v3/markers/counts');
        // Format for readability
        const summary = {
            maxUpdated: counts.max ? new Date(counts.max).toISOString() : null,
            unreadCounts: counts.unreadcounts?.map((c) => ({
                id: c.id,
                count: c.count,
...
server.tool("feedly_stream", "Get articles from a Feedly stream (feed, category, or all). Use feedly_collections to find stream IDs.", streamSchema, async (args) => {
    try {
        ...
        if (newerThan)
            params.append('newerThan', newerThan.toString());
        if (continuation)
        ...
        const articles = stream.items?.map((item) => ({
            id: item.id,
            title: item.title,
            author: item.author,
...
server.tool("feedly_all_articles", "Get your most recent articles across all subscriptions", {
    count: z.number().optional().default(20).describe('Number of articles to return (max 100)'),
...
// Tool: Mark as read
server.tool("feedly_mark_read", "Mark articles, feeds, or categories as read", markReadSchema, async (args) => {
    try {
...
server.tool("feedly_save_for_later", "Save an article to your 'Saved For Later' collection", {
...
server.tool("feedly_search_feeds", "Search for RSS feeds to potentially subscribe to", searchFeedsSchema, async (args) => {
    try {
        const params = new URLSearchParams({
            query: args.query,
        ...
        const feeds = results.results?.map((feed) => ({
            id: feed.id,
            title: feed.title,
...
server.tool("feedly_saved_articles", "Get articles you've saved for later", {
...
server.tool("feedly_info", "Get Feedly MCP server status and configuration info", {}, async () => {
