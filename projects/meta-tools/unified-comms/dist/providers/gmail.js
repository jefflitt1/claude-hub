...
function getOAuthConfig() {
    ...
    throw new Error('Google OAuth credentials not found. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET, ' +
 ...
 */
export async function initAccount(accountType) {
    ...
    try {
        ...
        const client = new google.auth.OAuth2(oauthConfig.clientId, oauthConfig.clientSecret, oauthConfig.redirectUri);
        // Load saved token
 ...
 */
export async function getGmailClient(accountType) {
 ...
 */
export function isAccountConfigured(accountType) {
    const config = getAccountConfig(accountType);
 ...
 */
export function getAuthUrl(accountType) {
    const oauthConfig = getOAuthConfig();
    const client = new google.auth.OAuth2(oauthConfig.clientId, oauthConfig.clientSecret, oauthConfig.redirectUri);
 ...
 */
export async function completeOAuthSetup(accountType, authorizationCode) {
    try {
        const config = getAccountConfig(accountType);
        const oauthConfig = getOAuthConfig();
        const client = new google.auth.OAuth2(oauthConfig.clientId, oauthConfig.clientSecret, oauthConfig.redirectUri);
 ...
 * Send an email
 */
export async function sendEmail(accountType, params) {
 ...
 */
export async function listMessages(accountType, count = 10) {
    const gmail = await getGmailClient(accountType);
    if (!gmail) {
        return { success: false, error: `${accountType} account not configured. Run OAuth setup first.` };
    }
    try {
        ...
        // Get message details
        const messageDetails = await Promise.all(messages.slice(0, count).map(async (msg) => {
 ...
 * Search messages
 */
export async function searchMessages(accountType, query) {
    const gmail = await getGmailClient(accountType);
    if (!gmail) {
        return { success: false, error: `${accountType} account not configured. Run OAuth setup first.` };
    }
 ...
 * Get message by ID
 */
export async function getMessage(accountType, messageId) {
    const gmail = await getGmailClient(accountType);
    if (!gmail) {
        return { success: false, error: `${accountType} account not configured. Run OAuth setup first.` };
    }
 ...
 */
export async function replyToMessage(accountType, params) {
    const gmail = await getGmailClient(accountType);
    if (!gmail) {
        return { success: false, error: `${accountType} account not configured. Run OAuth setup first.` };
    }
 ...
 * Get thread messages
 */
export async function getThread(accountType, threadId) {
    const gmail = await getGmailClient(accountType);
    if (!gmail) {
        return { success: false, error: `${accountType} account not configured. Run OAuth setup first.` };
    }
